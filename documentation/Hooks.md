...
### #3. Добавление через классы слушателей

Классы слушателей передаются в `Hook::configure()`. Каждый **публичный метод класса** становится
слушателем хука с тем же названием. Список классов явный: новый слушатель нужно добавить в него.

```php
\Expansa\Facades\Hook::configure(
    listeners: [
...
Вместо списка можно передать путь к файлу или каталогу, например для плагина с большим числом
слушателей: `Hook::configure(listeners: __DIR__ . '/Listeners')`. Тогда каталог сканируется, а имя
класса берётся из имени файла и его `namespace`.

Повторный вызов `configure()` для уже подключённого класса безопасен — слушатель не регистрируется
