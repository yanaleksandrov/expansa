...
final class View {
	...
	public static function redirect( string $location, int $status = 302, string $x_redirect_by = 'Grafema' ): bool|Error
	{
		 ...
		 */
		$location = Hook::call( 'grafema_redirect', $location, $status );
		 ...
		 * @param string $location the path or URL to redirect to
		 */
		$status = Hook::call( 'grafema_redirect_status', $status, $location );
	...
	public static function get( string $template, array $args = [] ) {
		 ...
		 */
		$filepath = Hook::call( 'grafema_view_part', $filepath, $template, $args );
