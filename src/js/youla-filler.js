(function() {
    ...
    const FILLER_PALETTE = null && PALETTE;
    ...
    class Filler {
        ...
        static computeMediaFilter(media) {
            return Filler.MEDIA_FILTERS.map(({key: key, css: css, unit: unit}) => `${css}(${media[key]}${unit})`).join(" ");
        }
        static clamp(value, min, max) {
            return Math.min(max, Math.max(min, value));
        }
        static el(tag, {part: part, ...props} = {}) {
        ...
        static isNearWhite({r: r, g: g, b: b}) {
        ...
        static rgbToHue({r: r, g: g, b: b}, max, d) {
            if (d === 0) {
                return 0;
            }
        ...
        static computePosition(anchorRect, size, viewport, offset = 6) {
            const {side: side} = Filler.availableSpace(anchorRect, viewport, offset);
            const position = {
                bottom: {
        ...
        constructor(target, options = {}) {
            ...
            const imageFilterDefaults = Object.fromEntries(Filler.MEDIA_FILTERS.map(({key: key, default: value}) => [ key, value ]));
            const videoSettingDefaults = Object.fromEntries(Filler.VIDEO_SETTINGS.map(({key: key, default: value}) => [ key, value ]));
            this.image = {
                dataUrl: null,
                fit: "cover",
                rotation: 0,
        ...
        initialize() {
            const {el: el, classes: classes} = this;
        ...
        addListeners() {
            const {el: el, alphaInput: alphaInput} = this;
        ...
        pickWithEyeDropper() {
            ...
            (new window.EyeDropper).open().then(({sRGBHex: sRGBHex}) => {
        ...
        bindAlphaSuffixDrag() {
            const {suffix: suffix, alphaInput: alphaInput} = this;
        ...
        render({skipHexInput: skipHexInput = false} = {}) {
            const {el: el, hsva: hsva, alphaInput: alphaInput} = this;
            const hex = this.hex;
        ...
        renderSwatch({skipHexInput: skipHexInput = false} = {}) {
            const {el: el, hsva: hsva, swatch: swatch, swatchColor: swatchColor, swatchColorOpaque: swatchColorOpaque, swatchVideo: swatchVideo, labels: labels} = this;
            ...
            if (isVideo) {
                const {dataUrl: dataUrl, fit: fit, rotation: rotation} = this.video;
                if (!skipHexInput) {
            ...
            if (isImage) {
                const {dataUrl: dataUrl, rotation: rotation} = this.image;
                if (!skipHexInput) {
        ...
        attachFloating(panel, content, onClose) {
            ...
            const reposition = () => {
                ...
                const {side: side, maxSize: maxSize} = Filler.availableSpace(anchorRect, viewport);
                ...
                const {top: top, left: left} = Filler.computePosition(anchorRect, size, viewport);
        ...
        openDropdown() {
            ...
            if (!this.dropdownBody) {
                const {host: host, root: root} = this.createShadowPanel(this.classes.dropdown);
        ...
        renderDropdown() {
            const {classes: classes, labels: labels} = this;
            ...
            [ ...this.palette ].sort((a, b) => a.name.localeCompare(b.name)).forEach(({name: name, hex: hex}) => paletteRow.appendChild(this.createPaletteItem(hex, name, false)));
        ...
        buildDialog() {
            ...
            const {host: host, root: dialog} = this.createShadowPanel(classes.dialog);
        ...
        syncSourceLabels() {
            const {dialogSourceButtons: dialogSourceButtons, labels: labels} = this;
        ...
        syncSourceUI() {
            const {dialogSourcesGroup: dialogSourcesGroup, dialogSourceButtons: dialogSourceButtons, dialogSolidPanel: dialogSolidPanel, dialogImagePanel: dialogImagePanel, dialogVideoPanel: dialogVideoPanel, sources: sources, source: source} = this;
        ...
        buildMediaPanel(type) {
            const {classes: classes, labels: labels} = this;
            const {tag: tag, accept: accept, classKeys: classKeys, labelKeys: labelKeys} = Filler.MEDIA[type];
        ...
        buildFilterSliders(type, cls, media) {
            ...
            const sliderInputs = {};
            const sliderValues = {};
            Filler.MEDIA_FILTERS.forEach(({key: key, min: min, max: max, step: step, default: def}) => {
        ...
        buildVideoSettings(type, cls, media) {
            ...
            const settingInputs = {};
            Filler.VIDEO_SETTINGS.forEach(({key: key, type: controlType, options: options}) => {
                const row = Filler.el("label", {
        ...
        hasMediaAdjustments(type) {
            const media = this[type];
            return type === "video" ? Filler.VIDEO_SETTINGS.some(({key: key, default: value}) => media[key] !== value) : Filler.MEDIA_FILTERS.some(({key: key, default: value}) => media[key] !== value);
        }
        ...
        handleMediaUpload(type, file) {
            const {accept: accept} = Filler.MEDIA[type];
        ...
        resetMediaAdjustments(type) {
            const media = this[type];
            if (type === "video") {
                const {settingInputs: settingInputs} = this.mediaRefs[type];
                Filler.VIDEO_SETTINGS.forEach(({key: key, type: controlType, default: value}) => {
                    media[key] = value;
                    if (controlType === "select") {
                ...
                const {sliderInputs: sliderInputs, sliderValues: sliderValues} = this.mediaRefs[type];
                Filler.MEDIA_FILTERS.forEach(({key: key, default: value}) => {
                    media[key] = value;
        ...
        renderMediaPreview(type) {
            const {dataUrl: dataUrl, fit: fit, rotation: rotation} = this[type];
            const {previewMedia: previewMedia, placeholder: placeholder, upload: upload, removeButton: removeButton, sliderInputs: sliderInputs} = this.mediaRefs[type];
        ...
        renderDialogFields() {
            ...
            const addField = (label, {min: min = 0, max: max, isHex: isHex = false} = {}, onInput) => {
                const input = Filler.el("input", {
                    type: "text",
        ...
        updateDialogFieldValues() {
            const {format: format, dialogFieldInputs: dialogFieldInputs} = this;
        ...
        renderDialog() {
            const {h: h, s: s, v: v, a: a} = this.hsva;
            const {dialogAreaHandle: dialogAreaHandle} = this;
        ...
        update(options = {}) {
            const {image: image, video: video, source: source, alpha: alpha, ...rest} = options;
            ...
            if ("disabled" in options) {
                const {disabled: disabled, el: el, alphaInput: alphaInput, swatch: swatch, wrapper: wrapper} = this;
